...
sprites.onOverlap(SpriteKind.Projectile, SpriteKind.MysteryBox, function (sprite, otherSprite) {
    ...
    if (Math.percentChance(100)) {
        EnemySquare = sprites.create(assets.image`Alien`, SpriteKind.Enemy)
        EnemySquare.setPosition(0, 16)
        EnemySquare.setVelocity(EnemyVel, 0)
        EnemySquare.setFlag(SpriteFlag.BounceOnWall, true)
    }
})
...
function SpawnMysterySupplyDrop (otherSprite: Sprite) {
    MysteryBox = sprites.create(assets.image`MysteryBox`, SpriteKind.MysteryBox)
    MysteryBox.x = otherSprite.x
    MysteryBox.y = otherSprite.y
    MysteryBox.setVelocity(0, 25)
    MysteryBox.setFlag(SpriteFlag.DestroyOnWall, true)
}
